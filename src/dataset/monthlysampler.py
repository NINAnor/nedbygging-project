...
class CustomGeoDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str = None):
        ...
        if stage == "fit" or stage is None:
            # Setup training data
            train_imgs = RasterDataset(paths=str(self.train_img_path), crs="epsg:32633", res=10)
            train_masks = RasterDataset(paths=str(self.train_mask_path), crs="epsg:32633", res=10)
        ...
        if stage == "validate" or stage is None:
            ...
            val_imgs = RasterDataset(paths=str(self.val_img_path), crs="epsg:32633", res=10)
            val_masks = RasterDataset(paths=str(self.val_mask_path), crs="epsg:32633", res=10)
        ...
        for sample in batch:
            ...
            if transform is not None:
                ...
                if isinstance(augmented['image'], np.ndarray):
                    sample['image'] = torch.from_numpy(augmented['image']).clone().detach()
                else:
                    sample['image'] = augmented['image'].clone().detach()

                if isinstance(augmented['mask'], np.ndarray):
                    sample['mask'] = torch.from_numpy(augmented['mask']).clone().detach().permute(2, 0, 1)
                else:
                    sample['mask'] = augmented['mask'].clone().detach().permute(2, 0, 1)
            else:
                sample['image'] = torch.tensor(image_np.transpose(2, 0, 1)).clone().detach()
